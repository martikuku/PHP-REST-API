...
class Api
{
    ...
    private static $main_version = 'v1.0'; // 1.0
    private static $main_format = 'xml';  // xml
    private static $request_version = null;
    private static $request_format = null;
    private static $public_routes = [
        // The configuration below will be used to enable requests to these URLs
        // /api/v1.0/stocks.json
        // /api/v1.0/stocks/7.json
        'stocks' => [
            'regex' => 'stocks',
        ],
        'stocks' => [
    ...
    public static function serve()
    {
        ...
        // Check whether the API server supports the requested version and format
        ...
        // Check version
        if (!in_array(self::$request_version, self::$supported_versions, true)) {
            throw new ErrorException('406 Unsupported version');
        }
        // Check format
        if (!in_array(self::$request_format, self::$supported_formats, true)) {
        ...
        if (!empty(self::$input)) {
            ...
            if (isset($matches[1]) && strpos(self::$input, $matches[1]) !== false) {
                self::parse_raw_request(self::$input, self::$input_data);
            } else {
        ...
        if ($request_method === 'options') {
            ...
            // $api_object = Stocks, 'get', 7       for /api/v1.0/stocks/7.json
            // $api_object = Stocks, 'get', null    for api/v1.0/stocks.xml
            // call_user_func_array(array($api_object, $request_method), $params_matches);
            $api_object->$request_method($params_matches, self::$input_data);
            // druhý parametr self::$input_data přidán kvůli post()
        }
    }
    ...
    public static function outputHeaders($cookies = [])
    {
    ...
    {
        if (self::$request_format === 'xml') {
            http_response_code(200);
            header('Content-type: text/xml; charset=utf-8');
            // self::outputHeaders($cookies);
            // https://phppot.com/php/php-restful-web-service/
            // creating object of SimpleXMLElement
            $xml_string = '<?xml version="1.0" encoding="Windows-1250"?><rsp:responsePack xmlns:rsp="http://www.stormware.cz/schema/response.xsd"note="response from the EShop" application="eShop" state="ok" id="00000001" version="1.0" ></rsp:responsePack>';
    ...
    public static function responseError($code = 404, $info = null)
    {
        ...
        if (self::$request_format === 'xml') {
            ...
            self::outputHeaders();
            $xml_string = '<?xml version="1.0" encoding="Windows-1250"?><rsp:responsePack xmlns:rsp="http://www.stormware.cz/schema/response.xsd"note="response from the EShop" application="eShop" state="error" id="00000001" version="1.0" ></rsp:responsePack>';
            $xml        = new SimpleXMLElement($xml_string, null, false);
            echo $xml->asXML();
        }
    }
}
